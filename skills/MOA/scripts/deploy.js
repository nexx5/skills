#!/usr/bin/env node
// MOA skill 部署脚本
// 功能：检查 moa-trigger.js plugin 是否已部署到全局，未部署则自动部署（复制 plugin、
//       注册 opencode.json plugin 数组、初始化 tasks.json），并提示重启 opencode。
// 用法：
...
const PLUGIN_SRC = path.join(__dirname, "moa-trigger.js")      // 本 skill 携带的 plugin 源文件

const OPENCODE_DIR = path.join(os.homedir(), ".config", "opencode")
const PLUGIN_DIR = path.join(OPENCODE_DIR, "plugin")
const PLUGIN_DEST = path.join(PLUGIN_DIR, "moa-trigger.js")
const OPENCODE_JSON = path.join(OPENCODE_DIR, "opencode.json")
const MOA_DIR = path.join(OPENCODE_DIR, "moa")
...
function deploy() {
  ...
  if (checkJsonRegistered()) {
    console.log(green(`[OK] opencode.json 已注册 moa-trigger.js`))
    ...
    cfg.plugin.push(PLUGIN_DEST.replace(/\\/g, "\\\\"))
  ...
  // 3. 初始化 tasks.json
  if (checkTasksInit()) {
    console.log(green(`[OK] ${TASKS_FILE} 已存在`))
  } else {
    if (!fs.existsSync(MOA_DIR)) fs.mkdirSync(MOA_DIR, { recursive: true })
    fs.writeFileSync(TASKS_FILE, "[]\n", "utf8")
    console.log(green(`[初始化] ${TASKS_FILE}`))
  }
}

console.log(`MOA skill 目录：${SKILL_DIR}`)
console.log(`目标 plugin：${PLUGIN_DEST}`)

const fileOk = checkPluginFile()
const jsonOk = checkJsonRegistered()
const tasksOk = checkTasksInit()
...
if (CHECK_ONLY) {
  ...
  console.log(tasksOk ? green("  [OK] tasks.json 已初始化") : red("  [缺失] tasks.json 未初始化"))
  console.log("")
  if (fileOk && jsonOk && tasksOk) {
    console.log(green("MOA 已就绪。"))
    process.exit(0)
  }
...
const finalTasks = checkTasksInit()

console.log("")
if (finalFile && finalJson && finalTasks) {
  console.log(green("✓ 部署完成，全部就绪。"))
  console.log(yellow(">>> 请重启 opencode 使 plugin 生效（plugin 仅加载时扫描一次）。重启后新会话应可见 moa_register 工具。"))
  process.exit(0)
}
